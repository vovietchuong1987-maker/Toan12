...
if(v34BaseTeacherAnalytics)v22RenderTeacherAnalytics=function(c){if(!c?.v34Partial)return v34BaseTeacherAnalytics(c);v22LastClassData=c;let cc=c,s=c.v34Snapshot||{};if(s.verifiedEvidence!=null&&s.verifiedAccuracy!=null){cc={...c,verifiedCompetencyV31:{schemaVersion:31,totalAttempts:Number(s.verifiedEvidence)||0,totalCredit:(Number(s.verifiedEvidence)||0)*Number(s.verifiedAccuracy),totalCorrect:0,codes:{},lessons:{},chapters:{},levels:{},accuracy:Number(s.verifiedAccuracy)}}}if(typeof v22RenderClassComparison==='function')v22RenderClassComparison(cc);let msg='<div class="teacher-live-empty">⚡ V35 không quét toàn bộ lớp ở lần mở đầu. Bấm “Tải phân tích đầy đủ” khi cần heatmap/xu hướng/gợi ý của toàn lớp.</div>';for(const id of ['v22ClassTrend','v22SkillHeatmap','v22AssignmentSuggestions']){let e=document.getElementById(id);if(e)e.innerHTML=msg}};
const v34BaseCompetencyRender=typeof v31RenderTeacherCompetency==='function'?v31RenderTeacherCompetency:null;
if(v34BaseCompetencyRender)v31RenderTeacherCompetency=function(c){if(!c?.v34Partial)return v34BaseCompetencyRender(c);v31LastClassData=c;let s=c.v34Snapshot||{},set=(id,v)=>{let e=document.getElementById(id);if(e)e.textContent=v};set('v31MetricEvidence',s.verifiedEvidence??'—');set('v31MetricAccuracy',s.verifiedAccuracy==null?'—':Math.round(Number(s.verifiedAccuracy)*100)+'%');set('v31MetricCoverage',s.verifiedCoverage??'—');set('v31MetricWeak','—');let html=`<div class="teacher-live-empty">⚡ Bản đồ Chương → Bài → Mã kiến thức cần tổng hợp toàn bộ studentStats. V35 để phần này ở chế độ lazy-load để tiết kiệm Reads.<br><br><button class="btn btn-blue" onclick="v34LoadFullTeacherAnalytics('${attrEsc(c.classId)}')">Tải phân tích đầy đủ</button></div>`;for(const id of ['v31LevelSummary','v31CompetencyHierarchy','v31StudentMatrix']){let e=document.getElementById(id);if(e)e.innerHTML=html}let n=document.getElementById('v31VerifiedNote');if(n)n.innerHTML='<b>V35 Scale:</b> các chỉ số tổng quan phía trên lấy từ snapshot lớp gần nhất; dữ liệu chi tiết chỉ đọc khi giáo viên yêu cầu.'};
