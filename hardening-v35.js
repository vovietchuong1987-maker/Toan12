...
function v35RunRegressionChecks({render=true,toast=false}={}){
  ...
  checks.push(v35Check('Phiên bản ứng dụng',String(APP_VERSION)==='38.2'&&meta==='38.2',`APP_VERSION=${APP_VERSION}; meta=${meta}; build=${build||V35_BUILD}`));
...
async function v35RegisterServiceWorker(){
  ...
  try{let reg=await navigator.serviceWorker.register('./sw-v38.2.1.js?v=38.2.1',{scope:'./',updateViaCache:'none'});v35ServiceWorkerState='ready';reg.update?.().catch(()=>{});v35RenderProductionCenter()}catch(err){v35ServiceWorkerState='error';v35CaptureIssue('service-worker',err)}
}
