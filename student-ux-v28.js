...
function v28Queue(){
  ...
  if(weak)q.push({priority:2.5,kind:'weak',title:`Củng cố ${weak.title||'kiến thức cần luyện'}`,meta:`${displayKnowledgeCode(weak.code)} • ${weak.total} lượt • ${weak.accuracy==null?'—':Math.round(weak.accuracy*100)+'%'}`,reason:'Đây là mã kiến thức có kết quả thấp trong lịch sử hiện tại.',minutes:Math.min(plan.dailyMinutes,25),code:weak.code});
