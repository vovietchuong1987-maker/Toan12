/* =========================================================
   Math12 Hub V39.1 — Mega Shop Expanded
...
(function(){
'use strict';
const BUILD='39.1-mega-shop-expanded',SCHEMA=391,PAGE_SIZE=40;
...
wardrobeSeries('hair','hair-v391','Tóc', '💇',{base:140,make:(i,color)=>({hairStyle:['short','side','spiky','bob','pony','long'][i%6],hairColor:color})});
wardrobeSeries('top','top-v391','Áo Math', '👕',{base:210,make:(i,color)=>({color,accent:colors[(i+5)%colors.length][1]})});
wardrobeSeries('bottom','bottom-v391','Quần/Váy', '👖',{base:180,make:(i,color)=>({color})});
wardrobeSeries('shoes','shoes-v391','Sneaker', '👟',{base:160,make:(i,color)=>({color})});
wardrobeSeries('head','head-v391','Mũ học giả', '🎓',{base:230,make:(i,color)=>({color,shape:i>=20?'crown':i%3===0?'cap':'hat'})});
wardrobeSeries('glasses','glasses-v391','Kính', '👓',{base:150,make:(i,color)=>({color})});
wardrobeSeries('back','back-v391','Ba lô', '🎒',{base:220,make:(i,color)=>({color})});
...
collections.forEach(([key,title,slots,icons],setIdx)=>slots.forEach((slot,i)=>{
 ...
 add({id:`collection-${key}-${slot}`,slot,label:`${title} • ${slotLabel(slot)}`,icon:icons[i],
   ...
   tool:['book','calculator','compass','ruler'][i%4],shape:i%3===0?'crown':'cap',
...
function persist(p,{cloud=true}={}){p={...p,schemaVersion:SCHEMA,ownerUid:uid(),updatedAt:now()};bucket()[uid()]=p;try{save({sync:false,reason:'mega-shop-v39.1'})}catch(_){};if(cloud&&firebaseUser&&firebaseDb&&!firebaseAccountLocked){firebaseDb.collection('users').doc(firebaseUser.uid).set({megaShopV386:p,updatedAt:firebaseServerTimestamp()},{merge:true}).catch(e=>console.warn('V39.1 shop sync',e))}render();return p}
...
function adoptCloud(){const c=firebaseProfile?.megaShopV386;if(!c)return;const l=profile(),ct=Date.parse(c.updatedAt||0)||0,lt=Date.parse(l.updatedAt||0)||0;if(ct>=lt){bucket()[uid()]={...blank(),...c,ownerUid:uid()};try{save({sync:false,reason:'mega-shop-cloud-v39.1'})}catch(_){}}}
