...
(function(){
  ...
  function v377PracticeForm(pattern,lessonId,title=''){
    ...
    const attemptType=`form-${pattern}`,first=!(state?.examAttempts||[]).some(a=>String(a?.type||'')===attemptType),difficulty=q=>{const d=Number(q?.difficulty);if(Number.isFinite(d))return Math.max(1,Math.min(5,d));const lv=String(q?.level||'').toUpperCase();return lv==='NB'?1.5:lv==='TH'?2.5:lv==='VD'?3.7:4.7};
    const count=Math.min(10,rows.length),seed=(Date.now()>>>0);let ordered=(typeof examShuffle==='function'?examShuffle(rows,seed):rows.slice());if(first)ordered=ordered.sort((a,b)=>difficulty(a)-difficulty(b));
    const pick=ordered.slice(0,count).map(q=>typeof normalizeBankQuestion==='function'?normalizeBankQuestion(q,'Luyện theo dạng ID6'):JSON.parse(JSON.stringify(q)));
    const cfg={id:`v377-form-${pattern.replace(/[^A-Za-z0-9]/g,'-')}-${new Date().toISOString().slice(0,10)}`,mode:'practice',title:`Luyện dạng • ${title||pattern}`,subtitle:`${pattern} • ${count} câu${first?' • khởi động từ dễ đến khó':''}`,durationMinutes:Math.max(10,count*2),questions:pick,scoring:'normalized',attemptType,rules:first?'Lượt đầu của dạng ưu tiên câu dễ trước để tạo động lực; sau đó hệ thống xáo trộn bình thường.':'Luyện đúng một dạng ID6. Đáp án được tự lưu trong quá trình làm bài.'};
