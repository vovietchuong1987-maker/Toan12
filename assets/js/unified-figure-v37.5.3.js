/* Math12 Hub V37.5.3 — Unified Figure Renderer
...
function patchFigureQC(){
  ...
  const wrapped=function(q={},opts={}){const r=old(q,opts);if((q.figureMode||'')==='tkztab'&&stored(q,norm(q.figureLatex||''))){r.renderEngine='stored-svg';r.kind='table';if(Array.isArray(r.checks)&&!r.checks.some(x=>x.name==='Unified tkz-tab SVG'))r.checks.push({name:'Unified tkz-tab SVG',ok:true,detail:'V37.5.3 ưu tiên Stored SVG; native BBT chỉ là fallback.',severity:'info'})}return r};wrapped.__v3753=true;api.qcQuestion=wrapped;
}
function regression(){
