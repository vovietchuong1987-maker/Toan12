/* =========================================================
   V25 — PRODUCTION ADMIN & USER MANAGEMENT
...
function v25AdminRequire(){if(isAdminRole?.())return true;alert('Chức năng này chỉ dành cho tài khoản Quản trị viên V25.');return false}
...
function renderAdminV25(){
  ...
  const b=page.querySelector('#v25AdminBanner');if(b){b.className='firebase-banner';b.textContent='V25 Admin Console đang dùng quyền Firestore đã xác thực. Khóa tài khoản là khóa truy cập ứng dụng/dữ liệu; vô hiệu hóa Firebase Authentication thật sự cần Admin SDK/backend.'}
  v25AdminRenderData();
}
...
async function v25AdminRefresh(force=true){
  ...
  }catch(err){console.error('V25 admin refresh',err);let b=document.getElementById('v25AdminBanner');if(b){b.className='firebase-banner error';b.textContent=firebaseErrorText(err)}}
...
function v25AdminRenderData(){
  ...
  v25AdminApplyFilters();v25AdminRenderClasses();v25AdminRenderAlerts();v25AdminRenderAudit();
...
async function v25AdminWriteAudit(action,targetType,targetId,detail={}){
  ...
  await firebaseDb.collection('adminAudit').add({actorUid:firebaseUser.uid,actorEmail:firebaseUser.email||'',action,targetType,targetId,detail,schemaVersion:25,createdAt:firebaseServerTimestamp()});
}
...
async function v25AdminToggleRole(uid,currentRole){
  ...
  try{let target=(v25AdminUsers||[]).find(x=>x.id===uid)||{},ref=firebaseDb.collection('users').doc(uid),b=firebaseDb.batch();b.update(ref,{role:next,accountStatus:target.accountStatus||'active',schemaVersion:25,adminUpdatedAt:firebaseServerTimestamp(),adminUpdatedBy:firebaseUser.uid,updatedAt:firebaseServerTimestamp()});let aref=firebaseDb.collection('adminAudit').doc();b.set(aref,{actorUid:firebaseUser.uid,actorEmail:firebaseUser.email||'',action:'user.role.change',targetType:'user',targetId:uid,detail:{from:currentRole,to:next},schemaVersion:25,createdAt:firebaseServerTimestamp()});await b.commit();await v25AdminRefresh(true)}catch(err){alert(firebaseErrorText(err))}
}

async function v25AdminToggleUserLock(uid,currentStatus){
  if(!v25AdminRequire())return;let target=(v25AdminUsers||[]).find(x=>x.id===uid);if(target?.role==='admin')return alert('V25 không cho khóa tài khoản admin từ trình duyệt.');
  ...
  try{let ref=firebaseDb.collection('users').doc(uid),b=firebaseDb.batch();b.update(ref,{accountStatus:next,schemaVersion:25,adminUpdatedAt:firebaseServerTimestamp(),adminUpdatedBy:firebaseUser.uid,updatedAt:firebaseServerTimestamp()});let aref=firebaseDb.collection('adminAudit').doc();b.set(aref,{actorUid:firebaseUser.uid,actorEmail:firebaseUser.email||'',action:next==='locked'?'user.lock':'user.unlock',targetType:'user',targetId:uid,detail:{status:next},schemaVersion:25,createdAt:firebaseServerTimestamp()});await b.commit();await v25AdminRefresh(true)}catch(err){alert(firebaseErrorText(err))}
}
...
async function v25AdminToggleClassAccess(classId,current){
  ...
  try{let ref=firebaseDb.collection('classes').doc(classId),b=firebaseDb.batch();b.update(ref,{accessStatus:next,schemaVersion:25,moderatedAt:firebaseServerTimestamp(),moderatedBy:firebaseUser.uid,updatedAt:firebaseServerTimestamp()});let aref=firebaseDb.collection('adminAudit').doc();b.set(aref,{actorUid:firebaseUser.uid,actorEmail:firebaseUser.email||'',action:next==='locked'?'class.lock':'class.unlock',targetType:'class',targetId:classId,detail:{accessStatus:next},schemaVersion:25,createdAt:firebaseServerTimestamp()});await b.commit();await v25AdminRefresh(true)}catch(err){alert(firebaseErrorText(err))}
}
...
function v25AdminRenderAlerts(){
  ...
  const oldSchema=(v25AdminUsers||[]).filter(u=>(Number(u.schemaVersion)||0)<25);if(oldSchema.length)alerts.push({level:'info',title:`${oldSchema.length} hồ sơ chưa lên schema V25`,text:'Hồ sơ sẽ tự cập nhật khi người dùng đăng nhập V25.'});
