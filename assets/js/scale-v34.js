...
async function v34CountQuery(q,kind='count'){
  try{if(q&&typeof q.count==='function'){let s=await v34Timed(kind,()=>q.count().get(),()=>0,true),d=typeof s.data==='function'?s.data():null,n=Number(d?.count??s?.count);return Number.isFinite(n)?n:null}}catch(err){console.warn('V34 count fallback',kind,err)}return null
}

/* Adaptive V19 cache: longer when tab is hidden, still invalidated on writes. */
...
/* Cross-session migration markers: first V34 visit can scan; later visits skip full scans. */
...
if(v34BaseNormalize)v27NormalizeAssignments=async function(classId){
  ...
  try{let scale={...(owned?.scaleV34||{}),teacherOpsNormalized:true,updatedAt:firebaseServerTimestamp()};await firebaseDb.collection('classes').doc(classId).set({scaleV34:scale,schemaVersion:34},{merge:true});if(owned){owned.scaleV34={...(owned.scaleV34||{}),teacherOpsNormalized:true}}}catch(err){console.warn('V34 normalize marker',err)}
};
...
/* Student list: newest 80 per target query, using V34 DESC indexes. */
...
/* Do not publish a class snapshot from a partial V34 page. */
...
if(v34BaseTeacherAnalytics)v22RenderTeacherAnalytics=function(c){if(!c?.v34Partial)return v34BaseTeacherAnalytics(c);v22LastClassData=c;let cc=c,s=c.v34Snapshot||{};if(s.verifiedEvidence!=null&&s.verifiedAccuracy!=null){cc={...c,verifiedCompetencyV31:{schemaVersion:31,totalAttempts:Number(s.verifiedEvidence)||0,totalCredit:(Number(s.verifiedEvidence)||0)*Number(s.verifiedAccuracy),totalCorrect:0,codes:{},lessons:{},chapters:{},levels:{},accuracy:Number(s.verifiedAccuracy)}}}if(typeof v22RenderClassComparison==='function')v22RenderClassComparison(cc);let msg='<div class="teacher-live-empty">⚡ V34 không quét toàn bộ lớp ở lần mở đầu. Bấm “Tải phân tích đầy đủ” khi cần heatmap/xu hướng/gợi ý của toàn lớp.</div>';for(const id of ['v22ClassTrend','v22SkillHeatmap','v22AssignmentSuggestions']){let e=document.getElementById(id);if(e)e.innerHTML=msg}};
const v34BaseCompetencyRender=typeof v31RenderTeacherCompetency==='function'?v31RenderTeacherCompetency:null;
if(v34BaseCompetencyRender)v31RenderTeacherCompetency=function(c){if(!c?.v34Partial)return v34BaseCompetencyRender(c);v31LastClassData=c;let s=c.v34Snapshot||{},set=(id,v)=>{let e=document.getElementById(id);if(e)e.textContent=v};set('v31MetricEvidence',s.verifiedEvidence??'—');set('v31MetricAccuracy',s.verifiedAccuracy==null?'—':Math.round(Number(s.verifiedAccuracy)*100)+'%');set('v31MetricCoverage',s.verifiedCoverage??'—');set('v31MetricWeak','—');let html=`<div class="teacher-live-empty">⚡ Bản đồ Chương → Bài → Mã kiến thức cần tổng hợp toàn bộ studentStats. V34 để phần này ở chế độ lazy-load để tiết kiệm Reads.<br><br><button class="btn btn-blue" onclick="v34LoadFullTeacherAnalytics('${attrEsc(c.classId)}')">Tải phân tích đầy đủ</button></div>`;for(const id of ['v31LevelSummary','v31CompetencyHierarchy','v31StudentMatrix']){let e=document.getElementById(id);if(e)e.innerHTML=html}let n=document.getElementById('v31VerifiedNote');if(n)n.innerHTML='<b>V34 Scale:</b> các chỉ số tổng quan phía trên lấy từ snapshot lớp gần nhất; dữ liệu chi tiết chỉ đọc khi giáo viên yêu cầu.'};

/* Invalidate V34 pages on writes. */
