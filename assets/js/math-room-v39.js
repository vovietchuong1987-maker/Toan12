...
(function(){
'use strict';
const BUILD='avatar-room-premium-v40.15.0',SCHEMA=400,CDN='https://cdn.babylonjs.com/babylon.js';
...
function createScene(){
  const p=profile(),t=currentTheme(),s=stats(),premium=window.Math12AvatarRoomPremium,env=premium?.environment?.(p)||{night:p.night,phase:p.night?'night':'day',exposure:p.night?.84:1.08,hemi:p.night?.42:.82,key:p.night?.48:1.15,window:p.night?.20:.62},perf=premium?.performanceProfile?.()||{shadowSize:1536,blurKernel:26};
  ...
  try{scene.imageProcessingConfiguration.toneMappingEnabled=true;scene.imageProcessingConfiguration.toneMappingType=BABYLON.ImageProcessingConfiguration.TONEMAPPING_ACES;scene.imageProcessingConfiguration.exposure=env.exposure;scene.imageProcessingConfiguration.contrast=1.04}catch(_){}
