...
(function(){
'use strict';
const BUILD='math-room-data-driven-v40.14.7',SCHEMA=400,CDN='https://cdn.babylonjs.com/babylon.js';
...
function install(){
  ...
  window.addEventListener('math12hub:room-data-changed',()=>{if(document.getElementById('page-room')?.classList.contains('active')){renderPage();setTimeout(mount,32)}});
...
window.v390Room={build:BUILD,schema:SCHEMA,themes,profile,stats,data:roomData,render:renderPage,mount,setTheme,toggleNight,focus,setAvatarPose,playAvatarMotion,destroy,refreshAvatar,refreshPet,refreshProgress,getScene:()=>scene,getAvatarRoot:()=>roomAvatarNode,getAvatarParts:()=>roomAvatarModel?.parts||null,getAvatarRenderer:()=>roomAvatarModel?.build||'legacy-room'};
