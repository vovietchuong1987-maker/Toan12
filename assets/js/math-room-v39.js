...
(function(){
'use strict';
const BUILD='room-presence-v40.14.9',SCHEMA=400,CDN='https://cdn.babylonjs.com/babylon.js';
...
function createScene(){
  ...
  buildFurniture(t,s,p);roomAvatarNode=roomAvatar();roomPetNode=roomPet();setTimeout(()=>{try{window.AvatarMotion?.attachContext?.('room');window.dispatchEvent(new CustomEvent('math12hub:avatar-renderer-mounted',{detail:{context:'room'}}));window.Math12RoomPresence?.mount?.(scene,window.v390Room);window.Math12RoomInteractions?.mount?.(scene,window.v390Room)}catch(err){console.warn('[Math Room] presence/interactions',err)}},0);
...
function destroy(){try{window.Math12RoomInteractions?.unmount?.()}catch(_){}try{window.Math12RoomPresence?.unmount?.()}catch(_){}try{window.AvatarMotion?.detachContext?.('room')}catch(_){}try{if(scene&&renderObs)scene.onBeforeRenderObservable.remove(renderObs)}catch(_){}renderObs=null;roomAvatarNode=null;roomAvatarModel=null;roomPetNode=null;shadowGen=null;try{scene?.dispose()}catch(_){};try{engine?.dispose()}catch(_){};scene=null;engine=null;canvas=null}
...
window.v390Room={build:BUILD,schema:SCHEMA,themes,profile,stats,data:roomData,render:renderPage,mount,setTheme,toggleNight,focus,setAvatarPose,playAvatarMotion,openInteraction:(kind)=>window.Math12RoomInteractions?.open?.(kind),closeInteraction:()=>window.Math12RoomInteractions?.close?.(),moveAvatar,destroy,refreshAvatar,refreshPet,refreshProgress,getScene:()=>scene,getAvatarRoot:()=>roomAvatarNode,getAvatarParts:()=>roomAvatarModel?.parts||null,getAvatarRenderer:()=>roomAvatarModel?.build||'legacy-room'};
