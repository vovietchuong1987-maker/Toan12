...
function renderContentReadiness(){
  ...
  box.innerHTML=`<div class="v395-content-head"><div><small>V40 • CONTENT PRODUCTION</small><h3>Độ sẵn sàng ngân hàng công khai</h3><p>Học sinh chỉ nhận câu Approved/Reviewed. Có thể thêm Chương 2–6 bằng content pack mà không sửa engine.</p></div><div><b>${r.approved}/${r.total}</b><span>câu đã phát hành</span></div></div><div class="v395-content-grid">${ch.map(c=>`<div class="v395-ch ${c.approved?'has':'empty'}"><b>Ch.${c.chapter}</b><strong>${c.approved}</strong><small>MCQ ${c.mcq} • Đ/S ${c.tf4} • Ngắn ${c.short}</small>${c.draft?`<em>${c.draft} Draft</em>`:''}</div>`).join('')}</div><div class="v395-content-foot">Content packs: <b>${r.packs.length}</b> • Câu bị chặn khỏi học sinh: <b>${r.excluded}</b></div>`;
}
...
function productionAudit(){
 ...
 add('Mega Shop runtime',!!window.v386MegaShop,'critical','V39+ UI owner');
