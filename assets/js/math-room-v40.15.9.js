...
function blank(){return {schemaVersion:SCHEMA,ownerUid:uid(),theme:'classroom',night:false,lighting:'auto',updatedAt:''}}
...
function profile(){const r=bucket()[uid()]||blank(),lighting=['auto','day','night'].includes(r.lighting)?r.lighting:(r.night?'night':'auto');return {...blank(),...r,ownerUid:uid(),theme:themes.some(t=>t.id===r.theme)?r.theme:'classroom',night:!!r.night,lighting}}
...
function roomData(){try{return window.Math12RoomData?.get?.()||null}catch(_){return null}}
function stats(){const d=roomData();if(d)return {achCount:d.achievements.count,mastered:d.mastery.mastered,lessonPass:d.learning.lessonPass,level:d.identity.level,books:d.visual.books,trophies:d.visual.trophies,roomLevel:d.visual.roomLevel,streak:d.learning.streak,averageScore:d.learning.recentAverageScore??d.learning.averageScore,lessonsCompleted:d.learning.lessonsCompleted,masteryAverage:d.mastery.average,recentAchievement:d.achievements.recent,chapters:d.chapters};const ach=window.v388Achievements?.profile?.().unlocked||{},achCount=Object.keys(ach).length,mastered=Number(window.v382Journey?.mastery?.()?.counts?.mastered)||0,lessonPass=Object.keys(window.v379Economy?.profile?.().rewardLedger?.lessonPass||{}).length,level=Number(window.v379Economy?.profile?.().level)||1,books=Math.min(18,Math.max(3,lessonPass+Math.floor(mastered/2))),trophies=Math.min(10,achCount),roomLevel=Math.min(5,1+Math.floor((achCount+mastered/2+level/5)/6));return {achCount,mastered,lessonPass,level,books,trophies,roomLevel,streak:0,averageScore:null,lessonsCompleted:0,masteryAverage:null,recentAchievement:null,chapters:[]}}
...
function createScene(){
  const p=profile(),t=currentTheme(),s=stats(),premium=window.Math12AvatarRoomPremium,env=premium?.environment?.(p)||{night:p.night,phase:p.night?'night':'day',exposure:p.night?.86:1.04,hemi:p.night?.44:.78,key:p.night?.46:1.03,window:p.night?.18:.56,fill:.20,rim:.18},perf=premium?.performanceProfile?.()||{shadowSize:1536,blurKernel:26};
  scene=new BABYLON.Scene(engine);const bg=c3(t.wall);scene.clearColor=new BABYLON.Color4(bg.r,bg.g,bg.b,1);scene.environmentIntensity=env.night?.72:.90;scene.metadata={...(scene.metadata||{}),math12RoomEnvironment:env.phase,premiumBuild:premium?.build||null};
  try{scene.imageProcessingConfiguration.toneMappingEnabled=true;scene.imageProcessingConfiguration.toneMappingType=BABYLON.ImageProcessingConfiguration.TONEMAPPING_ACES;scene.imageProcessingConfiguration.exposure=env.exposure;scene.imageProcessingConfiguration.contrast=1.025}catch(_){}
  ...
  const hemi=new BABYLON.HemisphericLight('roomHemi',new BABYLON.Vector3(.2,1,-.4),scene);hemi.intensity=env.hemi;hemi.diffuse=env.night?new BABYLON.Color3(.58,.68,.90):env.phase==='golden'?new BABYLON.Color3(1,.90,.78):new BABYLON.Color3(1,.95,.90);hemi.groundColor=c3(t.accent).scale(.22);
  const key=new BABYLON.DirectionalLight('roomKey',new BABYLON.Vector3(-.55,-1,.58),scene);key.position.set(-2.8,5.8,-4.2);key.diffuse=env.night?new BABYLON.Color3(.55,.66,1):env.phase==='golden'?new BABYLON.Color3(1,.78,.58):new BABYLON.Color3(1,.88,.72);key.intensity=env.key;
  const windowLight=new BABYLON.PointLight('windowLight',new BABYLON.Vector3(-3.2,2.55,-.1),scene);windowLight.diffuse=env.night?new BABYLON.Color3(.45,.60,1):env.phase==='morning'?new BABYLON.Color3(1,.88,.68):new BABYLON.Color3(1,.82,.58);windowLight.intensity=env.window;windowLight.range=6;
  const avatarFill=new BABYLON.PointLight('avatarFill',new BABYLON.Vector3(2.35,2.55,-2.25),scene);avatarFill.diffuse=env.night?new BABYLON.Color3(.54,.66,.90):new BABYLON.Color3(.92,.95,1);avatarFill.specular=avatarFill.diffuse.scale(.55);avatarFill.intensity=perf.lowPower?0:(env.fill??.20);avatarFill.range=4.6;
  const avatarRim=new BABYLON.PointLight('avatarRim',new BABYLON.Vector3(2.45,3.25,2.65),scene);avatarRim.diffuse=env.night?new BABYLON.Color3(.48,.62,1):new BABYLON.Color3(.78,.86,1);avatarRim.specular=avatarRim.diffuse.scale(.48);avatarRim.intensity=perf.lowPower?0:(env.rim??.18);avatarRim.range=4.8;
  try{shadowGen=new BABYLON.ShadowGenerator(perf.shadowSize,key);shadowGen.useBlurExponentialShadowMap=true;shadowGen.blurKernel=perf.blurKernel;shadowGen.bias=.0007}catch(_){shadowGen=null}
  const visualProfile={...p,night:!!env.night};buildFurniture(t,s,visualProfile);roomAvatarNode=roomAvatar();roomPetNode=roomPet();setTimeout(()=>{try{window.AvatarMotion?.attachContext?.('room');window.dispatchEvent(new CustomEvent('math12hub:avatar-renderer-mounted',{detail:{context:'room'}}));window.Math12RoomPresence?.mount?.(scene,window.v390Room);window.Math12RoomInteractions?.mount?.(scene,window.v390Room);window.Math12AvatarRoomPremium?.attachAura?.('room')}catch(err){console.warn('[Math Room] presence/interactions',err)}},0);
...
function stopRenderLoop(){if(engine&&renderLoop){try{engine.stopRenderLoop(renderLoop)}catch(_){}renderLoop=null}}
function startRenderLoop(){if(engine&&!renderLoop){renderLoop=()=>scene?.render();engine.runRenderLoop(renderLoop)}}
function destroy(){stopRenderLoop();try{window.Math12AvatarRoomPremium?.disposeAura?.()}catch(_){}try{window.Math12RoomInteractions?.unmount?.()}catch(_){}try{window.Math12RoomPresence?.unmount?.()}catch(_){}try{window.AvatarMotion?.detachContext?.('room')}catch(_){}try{if(scene&&renderObs)scene.onBeforeRenderObservable.remove(renderObs)}catch(_){}renderObs=null;roomAvatarNode=null;roomAvatarModel=null;roomPetNode=null;shadowGen=null;try{scene?.dispose()}catch(_){};try{engine?.dispose()}catch(_){};scene=null;engine=null;canvas=null}
function refreshAvatar(config=null,detail={}){
  if(!scene||!window.BABYLON)return false;
  disposeRoomAvatar();
  try{roomAvatarNode=roomAvatar();setTimeout(()=>{window.AvatarMotion?.attachContext?.('room');window.Math12RoomPresence?.rebind?.();window.Math12RoomInteractions?.rebind?.()},0);if((detail.changed||[]).includes('pet'))refreshPet();return true}catch(err){console.warn('room avatar refresh',err);return false}
}
function refreshPet(){
  if(!scene||!window.BABYLON)return false;
...
function recentAchievementHtml(s){
  const e=x=>typeof esc==='function'?esc(String(x??'')):String(x??'').replace(/[&<>\"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;'}[c]||c));
  const ach=s.recentAchievement;return ach?`<div class="v390-recent-ach"><span>${e(ach.icon||'🏆')}</span><div><small>Thành tích gần đây</small><b>${e(ach.title)}</b></div></div>`:`<div class="v390-recent-ach empty"><span>✦</span><div><small>Thành tích gần đây</small><b>Tiếp tục học để mở cột mốc đầu tiên</b></div></div>`;
}
function chapterRowsHtml(s){
  return (s.chapters||[]).map(c=>{const lp=Math.round((Number(c.lessonProgress)||0)*100),mp=c.masteryAverage==null?null:Math.round(Number(c.masteryAverage)*100);return `<div class="v390-chapter-row"><div><b>Chương ${c.id}</b><small>${c.completed}/${c.lessons} bài${mp==null?'':` • Mastery ${mp}%`}</small></div><strong>${lp}%</strong><i><em style="width:${lp}%"></em></i></div>`}).join('')||'<div class="v390-data-empty">Chưa có dữ liệu chương.</div>';
}
function refreshChrome(){
  ...
  set('roomLevel',`Lv.${s.roomLevel}`);set('books',String(s.books));set('trophies',String(s.trophies));set('mastered',String(s.mastered));set('roomLevelPlain',`${s.roomLevel}/5`);set('streak',String(s.streak||0));set('average',s.averageScore==null?'—':Number(s.averageScore).toFixed(1));set('lessons',String(s.lessonsCompleted||0));set('masteryAverage',s.masteryAverage==null?'—':Math.round(Number(s.masteryAverage)*100)+'%');
  ...
  const theme=root.querySelector('[data-room-theme]');if(theme)theme.textContent=t.label;const achBox=root.querySelector('[data-room-recent-ach]');if(achBox)achBox.innerHTML=recentAchievementHtml(s);const chaptersBox=root.querySelector('[data-room-chapters]');if(chaptersBox)chaptersBox.innerHTML=chapterRowsHtml(s);hud();
}
...
function hud(){const stage=document.querySelector('.v390-stage');if(!stage)return;const s=stats(),t=currentTheme(),tier=window.Math12AvatarRoomPremium?.auraTier?.(),env=window.Math12AvatarRoomPremium?.environment?.(profile());let x=stage.querySelector('.v390-stage-hud');if(!x){x=document.createElement('div');x.className='v390-stage-hud';stage.appendChild(x)}x.innerHTML=`<span>🏠 Lv.${s.roomLevel}</span><span>🔥 ${s.streak||0} ngày</span><span>📚 ${s.books} sách</span><span>🏆 ${s.trophies} thành tích</span>${tier?`<span class="v40150-aura-chip">✦ ${tier.label}</span>`:''}${env?`<span class="v40150-env-chip">${env.phase==='night'?'☾':env.phase==='morning'?'☀':'◐'} ${env.phase==='golden'?'Chiều ấm':env.phase==='morning'?'Buổi sáng':env.phase==='night'?'Buổi tối':'Ban ngày'}</span>`:''}<span data-room-theme>${t.label}</span>`}
...
function renderPage(){
  const root=document.getElementById('v390RoomPage');if(!root)return;const p=profile(),s=stats(),t=currentTheme();
  const e=x=>typeof esc==='function'?esc(String(x??'')):String(x??'').replace(/[&<>\"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;'}[c]||c));
  const achHtml=recentAchievementHtml(s),chapterHtml=chapterRowsHtml(s);
  root.innerHTML=`<div class="v390-room-shell"><section class="card v390-stage-card"><div class="v390-stage"><div class="v390-loading"><div class="v40150-room-skeleton"><i></i><i></i><i></i><strong></strong></div><div class="v40150-loading-copy"><b>Đang chuẩn bị không gian học tập…</b><span>Avatar • Mastery aura • ánh sáng thích ứng • tiến trình thật</span></div></div><canvas class="v390-canvas" aria-label="Phòng học 3D cá nhân"></canvas><div class="v390-controls"><button onclick="v390Room.focus('room')">⌂ Toàn phòng</button><button onclick="v390Room.focus('avatar')">☺ Nhân vật</button><button onclick="v390Room.focus('trophy')">🏆 Kệ thành tích</button><button onclick="v390Room.toggleNight()">${window.Math12AvatarRoomPremium?.label?.(p)|| (p.night?'☀ Ban ngày':'☾ Ban đêm')}</button></div></div></section><aside class="v390-side"><div class="v390-hero"><div class="avatar-preview-kicker">MY MATH ROOM</div><h2>Góc học tập của em</h2><p>Không gian cá nhân phản ánh trực tiếp nhịp học, Mastery và những cột mốc em đã đạt được.</p><div class="v390-room-level"><strong data-room-stat="roomLevel">Lv.${s.roomLevel}</strong><div><b>${e(window.avatarV378DisplayName?.()||'Học sinh Math12')}</b><small data-room-meta>Level ${s.level} • ${s.mastered} mã mastered • ${s.achCount} thành tích</small></div></div></div><div class="card v390-panel v390-learning-panel"><h3>Nhịp học của em</h3><p>Dữ liệu lấy từ các lượt học thật trên Math12 Hub, không phải chỉ số trang trí.</p><div class="v390-learning-grid"><div><small>Streak hiện tại</small><b><span data-room-stat="streak">${s.streak||0}</span> ngày</b></div><div><small>Điểm TB gần đây</small><b data-room-stat="average">${s.averageScore==null?'—':Number(s.averageScore).toFixed(1)}</b></div><div><small>Bài đã hoàn thành</small><b data-room-stat="lessons">${s.lessonsCompleted||0}</b></div><div><small>Mastery tổng hợp</small><b data-room-stat="masteryAverage">${s.masteryAverage==null?'—':Math.round(Number(s.masteryAverage)*100)+'%'}</b></div></div><div data-room-recent-ach>${achHtml}</div></div><div class="card v390-panel"><h3>Tiến độ theo chương</h3><p>Mỗi hàng phản ánh số bài đã hoàn thành; Mastery xuất hiện khi đã có đủ dữ liệu luyện tập.</p><div class="v390-chapters" data-room-chapters>${chapterHtml}</div></div><div class="card v390-panel"><h3>Phong cách căn phòng</h3><p>Đổi theme mà không ảnh hưởng điểm số. Theme nâng cao tiếp tục mở khóa từ Mega Shop.</p><div class="v390-themes">${themes.map(x=>{const ok=unlockedTheme(x),active=t.id===x.id;return `<button class="v390-theme ${active?'active':''} ${ok?'':'locked'}" ${ok?'':'disabled'} onclick="v390Room.setTheme('${x.id}')" style="--theme:${x.color}"><i></i><b>${x.label}</b><small>${x.free?'Có sẵn':ok?'Đã mở khóa':'Cần '+(window.v386MegaShop?.item?.(x.requires)?.label||'vật phẩm nền')}</small><em>${active?'Đang dùng':ok?'Có sẵn':'Khóa'}</em></button>`}).join('')}</div></div><div class="card v390-panel"><h3>Đồ trưng bày theo tiến độ</h3><div class="v390-summary"><div><small>Sách học tập</small><b data-room-stat="books">${s.books}</b></div><div><small>Cúp thành tích</small><b data-room-stat="trophies">${s.trophies}</b></div><div><small>Mã Mastery</small><b data-room-stat="mastered">${s.mastered}</b></div><div><small>Room Level</small><b data-room-stat="roomLevelPlain">${s.roomLevel}/5</b></div></div></div></aside></div>`;hud()
}
...
function install(){
  ...
  window.addEventListener('math12hub:room-data-changed',()=>{if(document.getElementById('page-room')?.classList.contains('active'))refreshProgress()});
  window.addEventListener('math12hub:avatar-runtime-ready',registerRendererBridge);
  window.addEventListener('resize',()=>engine?.resize());
  document.addEventListener('visibilitychange',()=>{if(!document.getElementById('page-room')?.classList.contains('active'))return;if(document.hidden)stopRenderLoop();else startRenderLoop()});
  if(document.getElementById('page-room')?.classList.contains('active')){renderPage();mount()}
}
window.v390Room={build:BUILD,schema:SCHEMA,themes,profile,stats,data:roomData,render:renderPage,mount,setTheme,toggleNight,focus,setAvatarPose,playAvatarMotion,openInteraction:(kind)=>window.Math12RoomInteractions?.open?.(kind),closeInteraction:()=>window.Math12RoomInteractions?.close?.(),moveAvatar,destroy,refreshAvatar,refreshPet,refreshProgress,getScene:()=>scene,getAvatarRoot:()=>roomAvatarNode,getAvatarParts:()=>roomAvatarModel?.parts||null,getAvatarRenderer:()=>roomAvatarModel?.build||'legacy-room',getEnvironment:()=>window.Math12AvatarRoomPremium?.environment?.(profile())||null};
