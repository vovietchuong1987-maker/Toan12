...
function v35RunRegressionChecks({render=true,toast=false}={}){
  ...
  checks.push(v35Check('Phiên bản ứng dụng',String(APP_VERSION)==='39.1'&&meta==='39.1',`APP_VERSION=${APP_VERSION}; meta=${meta}; build=${build||V35_BUILD}`));
...
async function v35RegisterServiceWorker(){
  ...
  try{let reg=await navigator.serviceWorker.register('./sw-v39.js?v=39.0.2',{scope:'./',updateViaCache:'none'});v35ServiceWorkerState='ready';reg.update?.().catch(()=>{});v35RenderProductionCenter()}catch(err){v35ServiceWorkerState='error';v35CaptureIssue('service-worker',err)}
}
