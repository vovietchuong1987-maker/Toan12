/* =========================================================
   Math12 Hub V37.3.5 — Production Hardening inherited from V35
...
const V35_BUILD='37.3.5-variation-exam-layout-polish';
const V35_FEATURES={
  ai:{src:'assets/js/ai-teacher-v32.js?v=37.3.5',label:'Trợ lý AI'},
  reports:{src:'assets/js/reports-v33.js?v=37.3.5',label:'Báo cáo học tập'},
...
async function v35EnsureFeature(name,{quiet=false}={}){
  ...
  let p=(async()=>{try{if(!quiet)v35SetFeatureLoading(true,`V37.3.5 đang tải ${f.label}…`);await v35LoadScript(f.src,{crossOrigin:f.crossOrigin});if(!v35FeatureReady(name))throw new Error(`${f.label} đã tải nhưng chưa khởi tạo được.`);v35RenderProductionCenter();return true}finally{if(!quiet)v35SetFeatureLoading(false)}})();
...
function v35RunRegressionChecks({render=true,toast=false}={}){
  ...
  checks.push(v35Check('Phiên bản ứng dụng',String(APP_VERSION)==='37.3.5'&&meta==='37.3.5',`APP_VERSION=${APP_VERSION}; meta=${meta}; build=${build||V35_BUILD}`));
  ...
  try{
    const rr=window.v3735VariationRegression?.();
    checks.push(v35Check('Variation & Exam Layout V37.3.5',rr?.ok===true,rr?.ok?`Bảng biến thiên ${rr.segments} đoạn neo cột • exam compact`:'V37.3.5 regression chưa đạt','fail'))
  }catch(err){checks.push(v35Check('Variation & Exam Layout V37.3.5',false,v35SanitizeErrorText(err?.message),'fail'))}
  ...
  v35RegressionLast={at:v35Now(),pass,warn,fail,checks};if(render)v35RenderProductionCenter();if(toast)examToast?.(fail?`V37.3.5: còn ${fail} lỗi kiểm tra`:`V37.3.5: ${pass} kiểm tra đạt${warn?`, ${warn} cảnh báo`:''}`);return v35RegressionLast
}
...
function v35ExportDiagnostics(){
  let payload=v35Diagnostics(),name=`math12hub-v37.3.5-diagnostics-${new Date().toISOString().slice(0,10)}.json`;
...
async function v35RegisterServiceWorker(){
  ...
  try{let reg=await navigator.serviceWorker.register('./sw-v37.3.5.js?v=37.3.5',{scope:'./',updateViaCache:'none'});v35ServiceWorkerState='ready';reg.update?.().catch(()=>{});v35RenderProductionCenter()}catch(err){v35ServiceWorkerState='error';v35CaptureIssue('service-worker',err)}
}
