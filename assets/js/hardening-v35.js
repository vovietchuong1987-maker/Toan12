...
const V35_FEATURES={
  ai:{src:'assets/js/ai-teacher-v32.js?v=40.13',label:'Trợ lý AI'},
...
function v35RunRegressionChecks({render=true,toast=false}={}){
  ...
  checks.push(v35Check('Phiên bản ứng dụng',String(APP_VERSION)==='40.13'&&meta==='40.13',`APP_VERSION=${APP_VERSION}; meta=${meta}; build=${build||V35_BUILD}`));
...
async function v35RegisterServiceWorker(){
  ...
  try{let reg=await navigator.serviceWorker.register('./sw.js?v=40.13',{scope:'./',updateViaCache:'none'});v35ServiceWorkerState='ready';reg.update?.().catch(()=>{});v35RenderProductionCenter()}catch(err){v35ServiceWorkerState='error';v35CaptureIssue('service-worker',err)}
}
