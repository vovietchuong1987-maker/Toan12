...
function v21FullBackupPayload(){return {format:'math12hub-full-backup',version:APP_VERSION,schemaVersion:31,createdAt:new Date().toISOString(),meta:{revision:Number(state._meta?.revision)||0,role:state.role||'student'},state:v21SafeStateSnapshot()}}
function v21BackupFilename(){const d=new Date(),p=n=>String(n).padStart(2,'0');return `math12hub-v27-full-${d.getFullYear()}${p(d.getMonth()+1)}${p(d.getDate())}-${p(d.getHours())}${p(d.getMinutes())}.json`}
...
function v21OpenFullRestorePreview(){
  ...
  const body=`<div class="notice"><b>${esc(p.fileName)}</b><br>Phiên bản ${esc(p.data.version||'?')} • ${p.data.createdAt?new Date(p.data.createdAt).toLocaleString('vi-VN'):''}</div><div class="backup-summary"><div><b>${bank}</b><small>Câu hỏi</small></div><div><b>${exams}</b><small>Đề đã lưu</small></div><div><b>${attempts}</b><small>Lượt kiểm tra</small></div></div><div class="restore-warnings"><b>V31 sẽ tạo điểm khôi phục trước khi thay dữ liệu.</b><br>Dữ liệu khôi phục được giữ ở máy trước; nếu đang đăng nhập, hệ thống sẽ đánh dấu cần đồng bộ lại thay vì âm thầm ghi đè cloud.</div>`;
