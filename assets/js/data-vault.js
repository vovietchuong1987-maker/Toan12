...
function v21OpenFullRestorePreview(){
  ...
  const body=`<div class="notice"><b>${esc(p.fileName)}</b><br>Phiên bản ${esc(p.data.version||'?')} • ${p.data.createdAt?new Date(p.data.createdAt).toLocaleString('vi-VN'):''}</div><div class="backup-summary"><div><b>${bank}</b><small>Câu hỏi</small></div><div><b>${exams}</b><small>Đề đã lưu</small></div><div><b>${attempts}</b><small>Lượt kiểm tra</small></div></div><div class="restore-warnings"><b>V22 sẽ tạo điểm khôi phục trước khi thay dữ liệu.</b><br>Dữ liệu khôi phục được giữ ở máy trước; nếu đang đăng nhập, hệ thống sẽ đánh dấu cần đồng bộ lại thay vì âm thầm ghi đè cloud.</div>`;
  openModal('Khôi phục toàn bộ dữ liệu','V22 • Data Safety',body,`<button class="btn btn-soft" onclick="closeModal()">Hủy</button><button class="btn btn-blue" onclick="v21CommitFullRestore()">Khôi phục</button>`);
}
