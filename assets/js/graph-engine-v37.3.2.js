/* ==========================================================
   Math12 Hub V37.3.2 — Native Function Graph Engine • Graph Visual Polish
   Supported families:
...
(function(){
'use strict';
const VERSION='37.3.2';
const BUILD='37.3.2-graph-visual-polish';
...
function defaultSpec(type='cubic'){
  const common={showGrid:false,showAsymptotes:true,showExtrema:true,showGuides:true,showPointLabels:false,showIntercepts:false,examStyle:true};
  if(type==='rational11')return {type,a:2,b:1,c:1,d:-1,...common};
  if(type==='rational21')return {type,a:1,b:-2,c:3,d:1,e:1,...common};
  return {type:'cubic',a:1,b:0,c:-3,d:-2,...common}
}
...
function specToText(s={}){
  ...
  rows.push(`xMin: ${Number.isFinite(s.xmin)?fmt(s.xmin):'auto'}`,`xMax: ${Number.isFinite(s.xmax)?fmt(s.xmax):'auto'}`,`yMin: ${Number.isFinite(s.ymin)?fmt(s.ymin):'auto'}`,`yMax: ${Number.isFinite(s.ymax)?fmt(s.ymax):'auto'}`,`grid: ${s.showGrid!==false}`,`examStyle: ${s.examStyle!==false}`,`asymptotes: ${s.showAsymptotes!==false}`,`extrema: ${s.showExtrema!==false}`,`guides: ${s.showGuides!==false}`,`pointLabels: ${!!s.showPointLabels}`,`intercepts: ${!!s.showIntercepts}`,`samples: ${s.samples||420}`);return rows.join('\n')
}
...
function autoBounds(s,a){
  ...
  if(!Number.isFinite(s.xmin)&&!Number.isFinite(s.xmax)){const xp=Math.max(.12,(xMax-xMin)*.045);xMin-=xp;xMax+=xp}if(Number.isFinite(s.xmin))xMin=s.xmin;if(Number.isFinite(s.xmax))xMax=s.xmax;if(!(xMax>xMin)){xMin=-5;xMax=5}
  ...
  if(!(yMax>yMin)){yMin-=2;yMax+=2}let pad=Math.max(.8,(yMax-yMin)*.18);yMin-=pad;yMax+=pad;
...
function graphSvg(specInput){
  const s=typeof specInput==='string'?parseSpec(specInput):{...defaultSpec(specInput?.type),...(specInput||{})},errors=validate(s);if(errors.length)return {ok:false,errors};
  const a=analyze(s),b=autoBounds(s,a),fn=functionFor(s),W=700,H=440,P={l:66,r:32,t:30,b:48},iw=W-P.l-P.r,ih=H-P.t-P.b,X=x=>P.l+(x-b.xMin)/(b.xMax-b.xMin)*iw,Y=y=>P.t+(b.yMax-y)/(b.yMax-b.yMin)*ih;
  const xStep=niceStep(b.xMax-b.xMin,9),yStep=niceStep(b.yMax-b.yMin,8),parts=[],defs=`<defs><marker id="v373arr" markerWidth="8" markerHeight="8" refX="7" refY="4" orient="auto"><path d="M0,0 L8,4 L0,8 z" fill="#111827"/></marker><clipPath id="v373plot"><rect x="${P.l}" y="${P.t}" width="${iw}" height="${ih}"/></clipPath></defs>`;
  parts.push(`<rect x="0" y="0" width="${W}" height="${H}" fill="#fff"/>`);
  if(s.showGrid!==false){for(let x=Math.ceil(b.xMin/xStep)*xStep;x<=b.xMax+1e-9;x+=xStep)parts.push(`<line x1="${X(x)}" y1="${P.t}" x2="${X(x)}" y2="${H-P.b}" stroke="#eef2f7" stroke-width="1"/>`);for(let y=Math.ceil(b.yMin/yStep)*yStep;y<=b.yMax+1e-9;y+=yStep)parts.push(`<line x1="${P.l}" y1="${Y(y)}" x2="${W-P.r}" y2="${Y(y)}" stroke="#eef2f7" stroke-width="1"/>`)}
  if(s.showAsymptotes!==false){a.vertical.forEach(x=>{if(x>b.xMin&&x<b.xMax)parts.push(`<line x1="${X(x)}" y1="${P.t}" x2="${X(x)}" y2="${H-P.b}" stroke="#64748b" stroke-width="1.2" stroke-dasharray="7 6"/>`)});a.horizontal.forEach(y=>{if(y>b.yMin&&y<b.yMax)parts.push(`<line x1="${P.l}" y1="${Y(y)}" x2="${W-P.r}" y2="${Y(y)}" stroke="#64748b" stroke-width="1.2" stroke-dasharray="7 6"/>`)});a.oblique.forEach(o=>parts.push(`<line x1="${X(b.xMin)}" y1="${Y(o.m*b.xMin+o.n)}" x2="${X(b.xMax)}" y2="${Y(o.m*b.xMax+o.n)}" stroke="#64748b" stroke-width="1.2" stroke-dasharray="7 6"/>`))}
  const axisY=(b.yMin<=0&&b.yMax>=0)?Y(0):H-P.b,axisX=(b.xMin<=0&&b.xMax>=0)?X(0):P.l;
  parts.push(`<line x1="${P.l}" y1="${axisY}" x2="${W-P.r+2}" y2="${axisY}" stroke="#111827" stroke-width="1.35" marker-end="url(#v373arr)"/>`,`<line x1="${axisX}" y1="${H-P.b}" x2="${axisX}" y2="${P.t-2}" stroke="#111827" stroke-width="1.35" marker-end="url(#v373arr)"/>`);
  const uniq=(vals,eps=1e-7)=>vals.filter(Number.isFinite).sort((u,v)=>u-v).filter((v,i,ar)=>!i||Math.abs(v-ar[i-1])>eps);
  function xTick(x,label=fmt(x,2)){if(!(x>b.xMin+1e-9&&x<b.xMax-1e-9)||Math.abs(x)<1e-9)return;const px=X(x),above=axisY>P.t+28;parts.push(`<line x1="${px}" y1="${axisY-3.5}" x2="${px}" y2="${axisY+3.5}" stroke="#111827" stroke-width="1"/>`,`<text x="${px}" y="${above?axisY-9:axisY+18}" text-anchor="middle" font-size="12" font-weight="500" fill="#111827">${escXml(label)}</text>`)}
  function yTick(y,label=fmt(y,2)){if(!(y>b.yMin+1e-9&&y<b.yMax-1e-9)||Math.abs(y)<1e-9)return;const py=Y(y),left=axisX>P.l+30;parts.push(`<line x1="${axisX-3.5}" y1="${py}" x2="${axisX+3.5}" y2="${py}" stroke="#111827" stroke-width="1"/>`,`<text x="${left?axisX-9:axisX+9}" y="${py+4}" text-anchor="${left?'end':'start'}" font-size="12" font-weight="500" fill="#111827">${escXml(label)}</text>`)}
  if(s.showGrid!==false){for(let x=Math.ceil(b.xMin/xStep)*xStep;x<=b.xMax+1e-9;x+=xStep)xTick(x);for(let y=Math.ceil(b.yMin/yStep)*yStep;y<=b.yMax+1e-9;y+=yStep)yTick(y)}
  else if(s.examStyle!==false){const sx=[...a.extrema.map(p=>p.x),...a.vertical];if(s.showIntercepts)sx.push(...a.intercepts.x.map(p=>p.x));const sy=[...a.extrema.map(p=>p.y),...a.horizontal];if(a.intercepts.y)sy.push(a.intercepts.y.y);uniq(sx).forEach(x=>xTick(x));uniq(sy).forEach(y=>yTick(y))}
  else{for(let x=Math.ceil(b.xMin/xStep)*xStep;x<=b.xMax+1e-9;x+=xStep)xTick(x);for(let y=Math.ceil(b.yMin/yStep)*yStep;y<=b.yMax+1e-9;y+=yStep)yTick(y)}
  parts.push(`<text x="${W-P.r+1}" y="${axisY+18}" text-anchor="end" font-size="16" font-style="italic" font-family="Georgia,serif" fill="#111827">x</text>`,`<text x="${axisX+10}" y="${P.t+3}" font-size="16" font-style="italic" font-family="Georgia,serif" fill="#111827">y</text>`);if(b.xMin<=0&&b.xMax>=0&&b.yMin<=0&&b.yMax>=0)parts.push(`<text x="${axisX-10}" y="${axisY+18}" text-anchor="end" font-size="12" font-family="Georgia,serif" fill="#111827">O</text>`);
  ...
  for(let i=0;i<N;i++){const x=b.xMin+(b.xMax-b.xMin)*i/(N-1);if(Number.isFinite(vert)&&Math.abs(x-vert)<(b.xMax-b.xMin)/N*1.6){flush();continue}let y=fn(x);if(!Number.isFinite(y)||y<b.yMin-(b.yMax-b.yMin)*2||y>b.yMax+(b.yMax-b.yMin)*2){flush();continue}if(prevY!=null&&Math.abs(Y(y)-Y(prevY))>ih*.7)flush();cur.push({x,y});prevY=y}flush();
  parts.push('<g clip-path="url(#v373plot)">');paths.forEach(ps=>parts.push(`<path d="${ps.map((p,i)=>`${i?'L':'M'} ${X(p.x).toFixed(2)} ${Y(p.y).toFixed(2)}`).join(' ')}" fill="none" stroke="#111827" stroke-width="2.65" stroke-linecap="round" stroke-linejoin="round"/>`));parts.push('</g>');
  const drawn=[];function already(p){return drawn.some(q=>Math.abs(q.x-p.x)<1e-7&&Math.abs(q.y-p.y)<1e-7)}
  function point(p,label=''){if(!p||p.x<b.xMin||p.x>b.xMax||p.y<b.yMin||p.y>b.yMax||already(p))return;drawn.push(p);if(s.showGuides){if(Math.abs(p.y)>1e-9&&b.yMin<0&&b.yMax>0)parts.push(`<line x1="${X(p.x)}" y1="${Y(p.y)}" x2="${X(p.x)}" y2="${Y(0)}" stroke="#475569" stroke-width="1.05" stroke-dasharray="6 5"/>`);if(Math.abs(p.x)>1e-9&&Math.abs(p.y)>1e-9&&b.xMin<0&&b.xMax>0)parts.push(`<line x1="${X(p.x)}" y1="${Y(p.y)}" x2="${X(0)}" y2="${Y(p.y)}" stroke="#475569" stroke-width="1.05" stroke-dasharray="6 5"/>`)}parts.push(`<circle cx="${X(p.x)}" cy="${Y(p.y)}" r="3.3" fill="#111827"/>`);if(s.showPointLabels&&label){const px=X(p.x),py=Y(p.y),right=px<W-P.r-95,above=py>P.t+28,tx=right?px+8:px-8,ty=above?py-9:py+17;parts.push(`<text x="${tx}" y="${ty}" text-anchor="${right?'start':'end'}" font-size="12" font-weight="600" fill="#111827">${escXml(label)}</text>`)}}
  if(s.showExtrema!==false)a.extrema.forEach(p=>point(p,`(${fmt(p.x,2)}; ${fmt(p.y,2)})`));a.holes.forEach(p=>{if(p.x>=b.xMin&&p.x<=b.xMax&&p.y>=b.yMin&&p.y<=b.yMax)parts.push(`<circle cx="${X(p.x)}" cy="${Y(p.y)}" r="5" fill="#fff" stroke="#111827" stroke-width="2"/>`)});if(s.showIntercepts){a.intercepts.x.forEach(p=>point(p,`(${fmt(p.x,2)}; 0)`));if(a.intercepts.y)point(a.intercepts.y,`(0; ${fmt(a.intercepts.y.y,2)})`)}
...
function toTikz(specInput){
  const s=typeof specInput==='string'?parseSpec(specInput):specInput,a=analyze(s),b=autoBounds(s,a),lines=[`\\begin{tikzpicture}[>=stealth,scale=0.8]`,`\\draw[->] (${fmt(b.xMin,2)},0) -- (${fmt(b.xMax,2)},0) node[below]{$x$};`,`\\draw[->] (0,${fmt(b.yMin,2)}) -- (0,${fmt(b.yMax,2)}) node[right]{$y$};`,`\\node[below left] at (0,0) {$O$};`];
  const uniq=(vals,eps=1e-7)=>vals.filter(Number.isFinite).sort((u,v)=>u-v).filter((v,i,ar)=>!i||Math.abs(v-ar[i-1])>eps);
  if(s.examStyle!==false&&s.showGrid===false){uniq([...a.extrema.map(p=>p.x),...a.vertical]).filter(x=>Math.abs(x)>1e-9).forEach(x=>lines.push(`\\node[above] at (${fmt(x,5)},0) {$${fmt(x,2)}$};`));uniq([...a.extrema.map(p=>p.y),...(a.intercepts.y?[a.intercepts.y.y]:[]),...a.horizontal]).filter(y=>Math.abs(y)>1e-9).forEach(y=>lines.push(`\\node[left] at (0,${fmt(y,5)}) {$${fmt(y,2)}$};`))}
  if(s.showAsymptotes!==false){a.vertical.forEach(x=>lines.push(`\\draw[thin,dashed] (${fmt(x,5)},${fmt(b.yMin,2)}) -- (${fmt(x,5)},${fmt(b.yMax,2)});`));a.horizontal.forEach(y=>lines.push(`\\draw[thin,dashed] (${fmt(b.xMin,2)},${fmt(y,5)}) -- (${fmt(b.xMax,2)},${fmt(y,5)});`));a.oblique.forEach(o=>lines.push(`\\draw[thin,dashed] (${fmt(b.xMin,2)},{${fmt(o.m,6)}*(${fmt(b.xMin,2)})+${fmt(o.n,6)}}) -- (${fmt(b.xMax,2)},{${fmt(o.m,6)}*(${fmt(b.xMax,2)})+${fmt(o.n,6)}});`))}
  ...
  if(s.showExtrema!==false)a.extrema.forEach(p=>{if(s.showGuides){if(Math.abs(p.y)>1e-9)lines.push(`\\draw[thin,dashed] (${fmt(p.x,5)},0) -- (${fmt(p.x,5)},${fmt(p.y,5)});`);if(Math.abs(p.x)>1e-9&&Math.abs(p.y)>1e-9)lines.push(`\\draw[thin,dashed] (${fmt(p.x,5)},${fmt(p.y,5)}) -- (0,${fmt(p.y,5)});`)}lines.push(`\\fill (${fmt(p.x,5)},${fmt(p.y,5)}) circle (1.5pt);`)});lines.push('\\end{tikzpicture}');return lines.join('\n')
}
...
function readBuilder(){const type=document.getElementById('v373Type')?.value||'cubic',s=defaultSpec(type);['a','b','c','d','e'].forEach(k=>{const el=document.getElementById('v373_'+k);if(el)s[k]=num(el.value,s[k]??0)});s.showGrid=!!document.getElementById('v373Grid')?.checked;s.examStyle=!!document.getElementById('v373ExamStyle')?.checked;s.showAsymptotes=!!document.getElementById('v373Asym')?.checked;s.showExtrema=!!document.getElementById('v373Extrema')?.checked;s.showGuides=!!document.getElementById('v373Guides')?.checked;s.showPointLabels=!!document.getElementById('v373Labels')?.checked;s.showIntercepts=!!document.getElementById('v373Intercepts')?.checked;return s}
...
function updateEditorHint(){const mode=document.getElementById('qeFigureMode')?.value,box=document.getElementById('qeFigureHint');if(mode!==MODE||!box)return;const raw=document.getElementById('qeFigureLatex')?.value||'',r=graphSvg(raw);box.innerHTML=r.ok?`<b>Graph V37.3.2:</b> ${escHtml(r.spec.type==='cubic'?'hàm bậc ba':r.spec.type==='rational11'?'phân thức bậc nhất/bậc nhất':'phân thức bậc hai/bậc nhất')} • SVG offline ✓${r.info.length?` • ${escHtml(r.info.join(' • '))}`:''}`:`<b>Graph V37.3.2:</b> <span class="bad">${escHtml(r.errors.join(' • '))}</span>`}
function installEditor(){
  const baseOpen=window.openQuestionEditor;window.openQuestionEditor=function(id=''){baseOpen(id);setTimeout(()=>{const sel=document.getElementById('qeFigureMode');if(!sel)return;if(![...sel.options].some(o=>o.value===MODE)){const opt=document.createElement('option');opt.value=MODE;opt.textContent='Đồ thị chuẩn THPT V37.3.2';const graphOpt=[...sel.options].find(o=>o.value==='graph2d');graphOpt?graphOpt.insertAdjacentElement('beforebegin',opt):sel.appendChild(opt)}const q=id?(state.questionBank||[]).find(x=>x.id===id):null;if(q?.figureMode===MODE)sel.value=MODE;const bar=document.querySelector('#qeFigureWrap .figure-toolbar');if(bar&&!document.getElementById('v373BuilderBtn')){const b=document.createElement('button');b.type='button';b.className='btn btn-soft';b.id='v373BuilderBtn';b.textContent='📈 Trình tạo đồ thị';b.onclick=openBuilder;bar.prepend(b)}toggleQuestionFigureFields?.();updateEditorHint();document.getElementById('qeFigureLatex')?.addEventListener('input',updateEditorHint);sel.addEventListener('change',updateEditorHint)},0)};
...
function installPresentation(){
  ...
  const baseName=window.figureModeName;window.figureModeName=function(mode='tikz'){return mode===MODE?'Đồ thị chuẩn THPT V37.3.2':baseName(mode)};
...
function regression(){
  ...
  ];let passed=0,details=[];for(const c of cases){const r=graphSvg(c.text),ok=r.ok&&c.expect(r)&&r.svg.includes('<path');if(ok)passed++;else details.push(c.name)}const f1=parseFormulaFamily('x^3-3x-2'),f2=parseFormulaFamily('(2x+1)/(x-3)'),f3=parseFormulaFamily('(x^2-2x+3)/(x+1)'),formulaOk=f1?.type==='cubic'&&f2?.type==='rational11'&&f3?.type==='rational21';const visual=graphSvg('type: cubic\na: 1\nb: 0\nc: -3\nd: -2\ngrid: false\nexamStyle: true\nextrema: true\nguides: true'),visualOk=visual.ok&&visual.bounds.yMin<=-4.7&&visual.bounds.yMin>=-5.1&&visual.bounds.yMax>=.6&&visual.bounds.yMax<=1.1&&['>-1<','>1<','>-2<','>-4<'].every(x=>visual.svg.includes(x))&&!visual.svg.includes('stroke=\"#eef2f7\"');return {ok:passed===cases.length&&formulaOk&&visualOk,passed,total:cases.length,formulaOk,visualOk,failed:details}
}
function init(){installPresentation();installEditor()}
window.v373RecognizeBuilderFormula=recognizeBuilderFormula;window.v373OpenGraphBuilder=openBuilder;window.v373OpenGraphAudit=openAudit;window.v373CloseGraphBuilder=closeBuilder;window.v373ApplyBuilder=applyBuilder;window.v373CopyTikz=copyTikz;window.v373GraphRegression=regression;window.V373Graph={version:VERSION,build:BUILD,mode:MODE,parseSpec,parseFormulaFamily,analyze,render:graphSvg,toTikz,specToText,defaultSpec,regression};window.V3732Graph=window.V373Graph;
init();
})();
