...
const ROLE_ACCESS={
  student:new Set(['dashboard','learning-plan','lessons','lesson-detail','chapters','periodic','thpt','progress','analytics','notifications','online']),
  teacher:new Set(['dashboard','lessons','lesson-detail','chapters','periodic','thpt','notifications','online','question-bank','ai-teacher','exam-builder','teacher']),
  admin:new Set(['dashboard','lessons','lesson-detail','chapters','periodic','thpt','notifications','online','question-bank','ai-teacher','exam-builder','teacher','admin'])
};
...
function goPage(page,internal=false){
  ...
  if(!canAccessPage(page,role)){
    if(!internal){let who=page==='admin'?'quản trị viên':['question-bank','ai-teacher','exam-builder','teacher'].includes(page)?'giáo viên':'học sinh',label=page==='question-bank'?'Ngân hàng câu hỏi':page==='ai-teacher'?'Trợ lý AI':page==='exam-builder'?'Tạo đề kiểm tra':page==='teacher'?'Theo dõi lớp':page==='admin'?'Quản trị hệ thống':'này';alert(`Mục “${label}” chỉ dành cho ${who}.`);}
    page='dashboard';
  }
  document.querySelectorAll('.section').forEach(s=>s.classList.remove('active'));let p=document.getElementById('page-'+page);if(p)p.classList.add('active');document.querySelectorAll('#nav button').forEach(b=>b.classList.toggle('active',b.dataset.page===page));let titles={dashboard:'Tổng quan học tập','learning-plan':'Lộ trình của em',lessons:'Học theo bài',chapters:'Ôn theo chương',periodic:'Kiểm tra định kỳ',thpt:'Ôn thi tốt nghiệp THPT',progress:'Tiến độ của em',analytics:'Phân tích năng lực','lesson-detail':'Chi tiết bài học','question-bank':'Ngân hàng câu hỏi','ai-teacher':'Trợ lý AI giáo viên','exam-builder':'Tạo đề kiểm tra',notifications:'Thông báo',online:'Lớp học online',teacher:'Theo dõi lớp',admin:'Quản trị hệ thống'};document.getElementById('pageTitle').textContent=titles[page]||'Math12 Hub';if(page==='learning-plan')v28RenderStudentUX?.();if(page==='progress')renderProgress();if(page==='analytics')renderAnalytics();if(page==='lesson-detail')renderLessonDetail();if(page==='question-bank'&&isTeacherRole())renderQuestionBank(true);if(page==='ai-teacher'&&isTeacherRole())v32RenderAIAssistant?.();if(page==='exam-builder'&&isTeacherRole()){renderExamBuilder(!document.getElementById('examBuilderMatrix')?.children.length);toggleExamBuilderTypes(false)}if(page==='notifications'){renderNotificationsPage();setTimeout(()=>firebaseRefreshNotifications(false),0)}if(page==='online')renderFirebaseOnlinePage();if(page==='teacher'&&isTeacherRole()){renderTeacher();setTimeout(()=>firebaseRefreshTeacherDashboard(false),0)}if(page==='admin'&&isAdminRole()){renderAdminV25?.();setTimeout(()=>v25AdminRefresh?.(false),0)}closeSidebar()
}
...
function renderQuestionBank(forceOptions=false){if(!requireTeacher('Ngân hàng câu hỏi'))return;
  ...
  const filtered=state.questionBank.filter(x=>(!cid||x.chapterId===cid)&&(!lid||x.lessonId===lid)&&(!kid||x.knowledgeCode===kid)&&(!lev||x.level===lev)&&(!typ||x.type===typ)&&(!q||[x.id,x.question,x.explanation,x.knowledgeCode,x.form,x.figureLatex||'',x.figureCaption||''].join(' ').toLowerCase().includes(q)));
...
function latexMeta(block){const meta={};block.replace(/^\s*%\s*(id|lesson|knowledge|level|form|source|year|tags|status|difficulty)\s*[:=]\s*(.+?)\s*$/gim,(_,k,v)=>{meta[k.toLowerCase()]=v.trim();return _});return meta}
function stripImportMeta(block){return block.replace(/^\s*%\s*(id|lesson|knowledge|level|form|source|year|tags|status|difficulty)\s*[:=].*$/gim,'').trim()}
...
function parseLatexExBlock(raw,index,defaults){
  ...
  let level=(meta.level||defaults.level||km.find(k=>k.code===knowledgeCode)?.level||'NB').toUpperCase();if(!['NB','TH','VD'].includes(level)){warnings.push(`Mức độ ${level} không hợp lệ, chuyển về NB.`);level='NB'}const lesson=getLesson(lessonId);
  let item={id:meta.id||`LATEX-${Date.now().toString(36).toUpperCase()}-${String(index+1).padStart(2,'0')}`,chapterId:lesson?.chapter.id||1,lessonId,knowledgeCode,form:meta.form||defaults.form||'',level,question:stem,explanation,figureMode:fig.figure?detectFigureMode(fig.figure,'tikz'):'none',figureLatex:fig.figure||'',figureCaption:'',figureLayout:immini?.layout||'below',source:'latex-import',sourceName:meta.source||'Import LaTeX',sourceYear:String(meta.year||'').replace(/[^0-9]/g,'').slice(0,4),tags:String(meta.tags||'').split(/[,;]/).map(x=>x.trim()).filter(Boolean).slice(0,12),reviewStatus:/^(reviewed|duyet|đã duyệt|da duyet)$/i.test(meta.status||'')?'reviewed':'draft',difficulty:Math.min(5,Math.max(1,Number(meta.difficulty)||({NB:2,TH:3,VD:4}[level]||3)))};
