...
function latexMeta(block){const meta={};block.replace(/^\s*%\s*(id|lesson|knowledge|level|form)\s*[:=]\s*(.+?)\s*$/gim,(_,k,v)=>{meta[k.toLowerCase()]=v.trim();return _});return meta}
function stripImportMeta(block){return block.replace(/^\s*%\s*(id|lesson|knowledge|level|form)\s*[:=].*$/gim,'').trim()}
