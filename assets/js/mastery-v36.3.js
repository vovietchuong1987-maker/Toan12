...
(function(){
  ...
  function startAdaptive(count=10,forcedCode=''){
    const plan=selectAdaptive(count,forcedCode);if(plan.questions.length<3){alert(`Ngân hàng chưa đủ câu đạt QC cho ${forcedCode||'các mã cần luyện'} (${plan.questions.length} câu khả dụng). Hãy bổ sung hoặc rà soát ngân hàng.`);return}
    const target=plan.targets.slice(0,4),targetText=target.map(x=>`${x.code} ${x.score==null?'mới':Math.round(x.score*100)+'%'}`).join(' • '),config={id:`adaptive-v363-${Date.now()}`,mode:'adaptive',attemptType:'adaptive-v363',title:forcedCode?`Luyện thích ứng • ${forcedCode}`:'Luyện thích ứng Mastery V36.3',subtitle:`${plan.questions.length} câu • ${targetText}`,durationMinutes:Math.max(10,Math.ceil(plan.questions.length*1.8)),questions:plan.questions.map(q=>normalizeBankQuestion(q,'Mastery V36.3')),scoring:'standard',adaptiveCodes:target.map(x=>x.code),v363:{build:BUILD,targets:target.map(x=>({code:x.code,mastery:x.score,confidence:x.confidence,targetDifficulty:x.targetDifficulty})),selectedAt:nowIso()}};openExamStart(config)
  }
  ...
  function injectUI(){
    ...
    if(!document.getElementById('v363TeacherMastery')){const v31=document.querySelector('#page-teacher .v31-competency-card');if(v31){const card=document.createElement('div');card.className='card mt v363-teacher-card';card.innerHTML='<div class="section-head" style="margin:0 0 10px"><div><span class="v360-kicker">V36.3 • MASTERY CLASS VIEW</span><h3>Mastery cá nhân hóa của lớp</h3><p>Tổng hợp snapshot học tập học sinh đã đồng bộ, có thể gồm cả kết quả Secure Exam đã nhập về thiết bị và tự luyện; không mở thêm collection và không tải submission nền.</p></div><span class="pill">Snapshot</span></div><div id="v363TeacherMastery"><div class="teacher-live-empty">Chọn lớp để xem Mastery đã đồng bộ.</div></div>';v31.insertAdjacentElement('afterend',card)}}
  }
