/* =========================================================
   ...
   - Filters the exam-builder pool by Question Quality Engine V36.1
   ...
   - Generates deterministic numeric codes 101–108 on top of V30 variant engine
...
(function(){
  ...
  function renderPreflight(){
    ...
    box.innerHTML=`<div class="v362-preflight-metrics"><span><b>${r.eligible}</b> câu đủ điều kiện</span><span><b>${r.safe}</b> không lỗi QC</span><span><b>${r.knowledge}</b> chuẩn kiến thức</span><span><b>${r.forms}</b> dạng toán</span></div><div class="v362-preflight-note">${r.short.length?`⚠ Có ${r.short.length} ô ma trận thiếu câu sau khi áp dụng bộ lọc V36.2.`:`✓ Ma trận hiện không thiếu câu theo bộ lọc V36.2.`} ${r.policy.avoidRecent?`Hệ thống ưu tiên tránh ${r.recent} câu đã dùng gần đây.`:'Không hạn chế câu đã dùng gần đây.'}</div>`;
  }
  function blueprint(){
  ...
  function decoratePreview(){
    const box=document.getElementById('examBuilderPreview');if(!box||!examBuilderDraft)return;updateCodeLabels();const p=typeof v30PolicyFromExam==='function'?v30PolicyFromExam(examBuilderDraft):(examBuilderDraft.v30Policy||{}),n=int(p.variantCount||1,1,8),strip=box.querySelector('.v30-policy-strip');if(strip){const lead=strip.querySelector('b');if(lead)lead.textContent=`V36.2 • ${n} mã ${examCode(0)}–${examCode(n-1)}`}
    if(!box.querySelector('.v362-smart-strip')){const s=examBuilderDraft.v362?.stats||{};const div=document.createElement('div');div.className='v362-smart-strip';div.innerHTML=`<div><b>Smart Matrix V36.2</b><span>QC TB ${s.avgQc||'—'}% • ${s.knowledgeCount||0} chuẩn • ${s.formCount||0} dạng • lặp gần đây ${s.repeated||0}</span></div><button class="btn btn-soft" onclick="v362ExportVariantPack()">⬇ Bộ mã ${examCode(0)}–${examCode(n-1)}</button>`;(strip||box.querySelector('.exam-preview-head'))?.insertAdjacentElement('afterend',div)}
  }
  ...
  function installHooks(){
    ...
    baseExamConfig=window.examBuilderConfig;if(typeof baseExamConfig==='function')window.examBuilderConfig=function(exam,variantIndex=0){const cfg=baseExamConfig(exam,variantIndex),code=examCode(variantIndex);cfg.internalVariantCode=cfg.variantCode;cfg.variantCode=code;cfg.examCode=code;cfg.subtitle=String(cfg.subtitle||'').replace(/Mã\s+[A-H0-9]+(?:\/\d+)?/i,`Mã ${code}`);cfg.rules=`V36.2 • Mã ${code}. `+(cfg.rules||'');return cfg};
