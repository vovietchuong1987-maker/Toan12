...
(function(){
'use strict';
const BUILD='40.16.0-avatar-engine-production',SCHEMA=600;
...
const API={build:BUILD,schema:SCHEMA,slots:ALL_SLOTS,face2:FACE2_CATALOG,get:()=>publicConfig(),base,resolved,effect,set:setBase,setHair,equip,apply:applyConfig,renderAvatar:applyConfig,preview,clearPreview,subscribe,ingest,adoptCloud,renderStatus,status:()=>{const c=publicConfig();return {build:BUILD,schema:SCHEMA,revision:Number(c.revision)||0,ownerUid:c.ownerUid||uid(),initialized:!!c.initialized,equipped:Object.values(c.equipped||{}).filter(Boolean).length}}};
