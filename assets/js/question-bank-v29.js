...
function v29QuestionQuality(q={}){
  ...
  if(['NB','TH','VD'].includes(q.level))score+=10;else issues.push('Thiếu mức độ');
...
function v29FilteredQuestions(){
  ...
  let rows=(state.questionBank||[]).filter(x=>(!cid||Number(x.chapterId)===cid)&&(!lid||x.lessonId===lid)&&(!kid||x.knowledgeCode===kid)&&(!lev||x.level===lev)&&(!typ||x.type===typ)&&(!status||x.reviewStatus===status)&&(!difficulty||Number(x.difficulty)===Number(difficulty))&&(!src||x.sourceName===src)&&(!tag||(x.tags||[]).some(t=>String(t).toLowerCase().includes(tag)))&&(!dup||(dup==='yes'?v29DuplicateCountFor(x.id)>0:v29DuplicateCountFor(x.id)===0))&&(!q||[x.id,x.question,x.explanation,x.knowledgeCode,x.form,x.sourceName,x.sourceYear,(x.tags||[]).join(' ')].join(' ').toLowerCase().includes(q)));
...
function exportQuestionBankCSVV29(){
  if(!requireTeacher('Xuất CSV ngân hàng'))return;const rows=v29FilteredQuestions(),head=['id','chapterId','lessonId','knowledgeCode','level','type','difficulty','reviewStatus','sourceName','sourceYear','tags','question','answer','explanation','version'];let csv='\uFEFF'+head.map(v29CsvCell).join(',')+'\n'+rows.map(q=>head.map(k=>{let v=q[k];if(k==='tags')v=(q.tags||[]).join('; ');if(k==='answer'&&q.type==='tf4')v=(q.statements||[]).map(s=>s.answer?'Đ':'S').join('');if(k==='answer'&&q.type==='mcq')v=String.fromCharCode(65+(Number(q.answer)||0));return v29CsvCell(v)}).join(',')).join('\n');const a=document.createElement('a');a.href=URL.createObjectURL(new Blob([csv],{type:'text/csv;charset=utf-8'}));a.download=`math12-question-bank-v29-${rows.length}-cau.csv`;a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000)
}
function v29QuestionToLatex(q={}){
  const meta=[`% id: ${q.id}`,`% lesson: ${q.lessonId}`,`% knowledge: ${q.knowledgeCode}`,`% level: ${q.level}`,q.form?`% form: ${q.form}`:'',q.sourceName?`% source: ${q.sourceName}`:'',q.sourceYear?`% year: ${q.sourceYear}`:'',q.tags?.length?`% tags: ${q.tags.join(', ')}`:''].filter(Boolean).join('\n');
