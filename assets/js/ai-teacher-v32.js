...
async function v32AiClearKey(){if(!requireTeacher('Xóa API key'))return;if(!await Math12UI.confirm('Xóa API key Gemini đã lưu trên trình duyệt này?'))return;localStorage.removeItem(V32_AI_KEY_LOCAL);sessionStorage.removeItem(V32_AI_KEY_SESSION);v32RenderAiStatus();examToast?.('Đã xóa API key Gemini.')}
...
async function v32AiExtractQuestions(){
  if(!requireTeacher('AI tạo bản nháp'))return;const text=(document.getElementById('v32AiSourceText')?.value||'').trim(),file=v32AiSelectedFile,count=Math.min(10,Math.max(1,Number(document.getElementById('v32AiCount')?.value)||3)),typePolicy=document.getElementById('v32AiTypePolicy')?.value||'mixed';if(!text&&!file)return Math12UI.alert('Hãy dán nội dung/LaTeX hoặc chọn ảnh/PDF.');
...
async function v32AiAuditSelected(){
  if(!requireTeacher('AI kiểm định câu hỏi'))return;const q=v32BankQuestionByInput();if(!q)return Math12UI.alert('Nhập/chọn đúng mã câu trong ngân hàng.');const box=document.getElementById('v32AiAuditResult');if(box)box.innerHTML='<div class="v32-ai-loading"><span class="v32-ai-spinner"></span> AI đang kiểm tra đáp án, cấu trúc và phân loại…</div>';
...
async function v32ApproveDraft(draftId,reviewed=false){
  if(!requireTeacher('Duyệt bản nháp AI'))return;const d=v32AiDrafts.find(x=>x.draftId===draftId);if(!d)return;const q=v32JsonClone(d.question),check=v32LocalDraftChecks(q);if(check.issues.length&&!await Math12UI.confirm(`Câu còn ${check.issues.length} cảnh báo local:\n- ${check.issues.join('\n- ')}\n\nVẫn đưa vào ngân hàng?`))return;if(reviewed&&!await Math12UI.confirm('Xác nhận thầy/cô đã tự kiểm tra nội dung, đáp án và lời giải để đánh dấu “Đã duyệt chuyên môn”?'))return;q.id=v32UniqueBankId(q.id);q.reviewStatus=reviewed?'reviewed':'draft';q.updatedAt=v32Now();q.createdAt=q.createdAt||v32Now();q.aiV32={...(q.aiV32||{}),teacherReviewed:true,teacherDecision:reviewed?'reviewed':'accepted-draft',approvedAt:v32Now()};state.questionBank.unshift(v29NormalizeQuestion(q));save({reason:reviewed?'v32-ai-approved-reviewed':'v32-ai-approved-draft'});v32AiDrafts=v32AiDrafts.filter(x=>x.draftId!==draftId);v32AiPersistDrafts();v29DuplicateCache.signature='';v32RenderAiDraftQueue();v32RenderAiQuestionPicker();v32RenderAiMetrics();renderQuestionBank(true);examToast?.(`Đã đưa ${q.id} vào ngân hàng${reviewed?' và đánh dấu đã duyệt':''}.`)
}
async function v32DiscardDraft(id){if(!await Math12UI.confirm('Bỏ bản nháp AI này?'))return;v32AiDrafts=v32AiDrafts.filter(x=>x.draftId!==id);v32AiPersistDrafts();v32RenderAiDraftQueue();v32RenderAiMetrics()}
...
async function v32ClearDraftQueue(){if(!v32AiDrafts.length)return;if(!await Math12UI.confirm(`Xóa ${v32AiDrafts.length} bản nháp AI đang chờ? Việc này không ảnh hưởng câu đã vào ngân hàng.`))return;v32AiDrafts=[];v32AiPersistDrafts();v32RenderAiDraftQueue();v32RenderAiMetrics()}
...
async function v4013ResetPipeline(){if(v4013Job&&!await Math12UI.confirm('Đặt lại checkpoint AI Import Pipeline? Các bản nháp trong hàng chờ vẫn được giữ nguyên.'))return;v4013Job=null;v4013StopRequested=true;localStorage.removeItem(V4013_PIPELINE_KEY);v4013RenderPipeline();examToast?.('Đã đặt lại tiến trình AI Import Pipeline.')}
...
async function v40135UploadDraftFigure(draftId,input){const file=input?.files?.[0];if(!file)return;if(!/^image\//i.test(file.type||''))return Math12UI.alert('Chỉ nhận tệp ảnh.');const dataUrl=await v40135ReadFileAsDataUrl(file);const info=await v40135ImageInfo(dataUrl).catch(()=>({width:0,height:0}));v40135SetDraftImageData(draftId,{dataUrl,name:file.name,label:file.name,kind:'manual-upload',width:info.width,height:info.height,bytes:Math.round((dataUrl.length*3)/4)});closeModal();examToast?.('Đã gắn hình cho câu trong Hàng chờ AI.');}
...
function v40135EnhanceDraftActions(){
  ...
  box.querySelectorAll('.v32-draft-card').forEach(card=>{
    ...
    let clear=act.querySelector('.v40135-clear-btn');if(q.figureMode==='image'&&q.figureImageData){if(!clear){clear=document.createElement('button');clear.className='btn btn-soft v40135-clear-btn';clear.textContent='Bỏ hình';clear.onclick=async()=>{if(await Math12UI.confirm('Bỏ hình đang gắn cho bản nháp này?'))v40135ClearDraftImage(id)};act.insertBefore(clear,pick?.nextSibling||null)}if(!card.querySelector('.v40135-draft-chip')){const meta=card.querySelector('.v32-draft-meta');if(meta){const span=document.createElement('span');span.className='v40135-draft-chip';span.textContent='🖼 có hình gốc';meta.appendChild(span)}}}
...
const v40135BaseSaveEditor=window.saveQuestionEditor;window.saveQuestionEditor=async function(editId=''){
  ...
  const extracted=extractTikzFromText(question);question=(extracted.text||question).trim();if(!question){Math12UI.alert('Cần nhập nội dung câu hỏi.');return}
  ...
  if(!figureImageData){Math12UI.alert('Đã chọn chế độ hình ảnh nhưng chưa gắn ảnh.');return}
  ...
  if(type==='mcq'){let opts=document.getElementById('qeOptions').value.split('\n').map(x=>x.trim()).filter(Boolean);if(opts.length<2){Math12UI.alert('Câu nhiều lựa chọn cần ít nhất 2 phương án.');return}let ai='ABCD'.indexOf(rawAns.toUpperCase());if(ai<0||ai>=opts.length){Math12UI.alert('Đáp án câu nhiều lựa chọn cần là A, B, C hoặc D tương ứng phương án.');return}item.options=opts;item.answer=ai}else if(type==='tf4'){let lines=(document.getElementById('qeTF4Statements')?.value||'').split('\n').map(x=>x.trim()).filter(Boolean);if(lines.length!==4){Math12UI.alert('Câu Đúng/Sai 4 ý cần đúng 4 dòng.');return}let exps=(document.getElementById('qeTF4Explanations')?.value||'').split('\n');item.statements=lines.map((s,i)=>({text:s.replace(/\\True\b/g,'').trim(),answer:/\\True\b/.test(s),explanation:(exps[i]||'').trim()}))}else if(type==='tf'){let a=rawAns.toLowerCase();if(!['đúng','dung','true','sai','false'].includes(a)){Math12UI.alert('Đáp án Đúng/Sai hãy nhập “Đúng” hoặc “Sai”.');return}item.answer=['đúng','dung','true'].includes(a)}else{if(!rawAns){Math12UI.alert('Cần nhập đáp án ngắn.');return}item.answer=rawAns}
  const latexCheck=validateQuestionLatexItem(item);if(latexCheck.errors.length){Math12UI.alert('Câu hỏi còn lỗi LaTeX cần sửa trước khi lưu:\n- '+latexCheck.errors.join('\n- '));return}if(latexCheck.warnings.length&&!await Math12UI.confirm('Có một số cảnh báo LaTeX:\n- '+latexCheck.warnings.join('\n- ')+'\n\nVẫn lưu câu hỏi?'))return;
  ...
  if(state.questionBank.some(q=>q.id===item.id&&q.id!==editId)){Math12UI.alert('Mã câu đã tồn tại.');return}
...
async function confirmAllAuto(){if(!requireTeacher?.('Xác nhận ID6 hàng loạt'))return;const rows=(v32AiDrafts||[]).filter(d=>id6Status(d.question)==='auto');if(!rows.length)return Math12UI.alert('Không có câu ID6 tự động để xác nhận.');if(!await Math12UI.confirm(`Xác nhận ID6 cho ${rows.length} câu đang ở trạng thái “ID6 tự động”?\n\nViệc này chỉ xác nhận phân loại ID6/metadata; không publish câu.`))return;rows.forEach(d=>{d.question=confirmPattern(d.question,d.question.id6Auto?.pattern||d.question.id6Pattern,'bulk-auto')});saveDrafts();examToast?.(`Đã xác nhận ID6 cho ${rows.length} câu.`)}
...
v32AiClearKey=async function(){if(!requireTeacher('Xóa API key'))return;v40140EnsureLegacyMigration();const e=v40140ActiveEntry({allowBlocked:true});if(!e)return v40140LegacyClearKey?.();if(!await Math12UI.confirm(`Xóa “${e.name||'API'}” khỏi Multi-Key Pool?`))return;v40140RemoveKey(e.id);examToast?.('Đã xóa key đang dùng khỏi Multi-Key Pool.')};
...
function v40140UseKey(id){const e=v40140Entry(id);if(!e)return;if(!v40140Secret(e))return Math12UI.alert('Key này không còn secret trong phiên/thiết bị. Hãy xóa và thêm lại.');v40140SetActiveId(id);v40140SetActionStatus(`Đang dùng ${e.name} • ${e.group}.`,'ok')}
...
async function v40140DeleteKey(id){const e=v40140Entry(id);if(!e)return;if(!await Math12UI.confirm(`Xóa “${e.name}” khỏi Pool?`))return;v40140RemoveKey(id);v40140SetActionStatus(`Đã xóa ${e.name}.`,'')}
async function v40140EditKey(id){const e=v40140Entry(id);if(!e)return;const name=await Math12UI.prompt('Tên API:',e.name||'API');if(name===null)return;const group=await Math12UI.prompt('Nhóm Project (các key cùng Google project phải nhập cùng tên nhóm):',e.group||'Project 1');if(group===null)return;v40140UpdateEntry(id,{name:String(name||e.name).trim(),group:v40140DisplayGroup(group||e.group),authBlocked:false,lastStatusText:'Đã cập nhật thông tin'});v40140RenderPool();v40140RenderStatus()}
