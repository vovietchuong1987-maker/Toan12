/* =========================================================
   V27 — PRODUCTION ADMIN + SYSTEM HEALTH
...
function v25AdminRequire(){if(isAdminRole?.())return true;alert('Chức năng này chỉ dành cho tài khoản Quản trị viên V27.');return false}
...
function renderAdminV25(){
  ...
  const b=page.querySelector('#v25AdminBanner');if(b){b.className='firebase-banner';b.textContent='V27 Admin Console đang dùng quyền Firestore đã xác thực. Khóa tài khoản là khóa truy cập ứng dụng/dữ liệu; vô hiệu hóa Firebase Authentication thật sự cần Admin SDK/backend.'}
  v25AdminRenderData();
}
...
async function v25AdminRefresh(force=true){
  ...
  }catch(err){console.error('V27 admin refresh',err);let b=document.getElementById('v25AdminBanner');if(b){b.className='firebase-banner error';b.textContent=firebaseErrorText(err)}}
...
async function v25AdminToggleUserLock(uid,currentStatus){
  if(!v25AdminRequire())return;let target=(v25AdminUsers||[]).find(x=>x.id===uid);if(target?.role==='admin')return alert('V27 không cho khóa tài khoản admin từ trình duyệt.');
...
function v25AdminRenderAlerts(){
  ...
  const oldSchema=(v25AdminUsers||[]).filter(u=>(Number(u.schemaVersion)||0)<27);if(oldSchema.length)alerts.push({level:'info',title:`${oldSchema.length} hồ sơ chưa lên schema V27`,text:'Hồ sơ sẽ tự cập nhật khi người dùng đăng nhập V27.'});
...
function v25AdminRenderAudit(){
  const box=document.getElementById('v25AdminAudit');if(!box)return;const actionName={'user.role.change':'Đổi quyền','user.lock':'Khóa tài khoản','user.unlock':'Mở tài khoản','user.password-reset.request':'Gửi reset mật khẩu','class.lock':'Khóa lớp','class.unlock':'Mở lớp','system.integrity.repair':'Sửa toàn vẹn V27'};
  box.innerHTML=(v25AdminAudit||[]).length?(v25AdminAudit||[]).map(a=>`<div class="audit-row"><div><b>${esc(actionName[a.action]||a.action||'Thao tác')}</b><small>${esc(a.actorEmail||a.actorUid||'admin')} → ${esc(a.targetType||'')} ${esc(a.targetId||'')}</small></div><span>${esc(firebaseDateText?.(a.createdAt)||'')}</span></div>`).join(''):'<div class="online-empty">Chưa có nhật ký quản trị V27.</div>';
}
