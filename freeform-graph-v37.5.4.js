/* Math12 Hub V37.5.4 — Native Freeform Graph Renderer
   - Stored SVG remains highest priority via V37.5.3.
...
function render(tex=''){
  const p=parseTikz(tex);if(!p.ok||(!p.freeform&&!p.foreach))return {ok:false,reason:p.reason||'Không phải đồ thị freeform V37.5.4.',unsupported:p.unsupported||[]};if(p.unsupported.length)return {ok:false,reason:'Còn lệnh TikZ chưa hỗ trợ native.',unsupported:p.unsupported};
...
function wrap(item={},r={},compact=false){const d=item.figureDisplay&&typeof item.figureDisplay==='object'?item.figureDisplay:{},mw=clamp(Number(d.maxWidth)||650,320,980),mh=clamp(Number(d.maxHeight)||500,220,720),align=['left','right','center'].includes(d.align)?d.align:'center',cap=item.figureCaption?`<div class="latex-figure-caption">${typeof window.mathHTML==='function'?window.mathHTML(item.figureCaption):esc(item.figureCaption)}</div>`:'';return `<div class="latex-figure ${compact?'compact ':''}v3745-figure v3754-freeform ${align}" data-v3745-engine="native-freeform-svg" data-v3745-kind="graph-freeform" data-v3745-crop="tight" data-v3754-engine="native-freeform-svg" style="--v3745-max-w:${mw}px;--v3745-max-h:${mh}px"><div class="v3754-engine"><span>Native Freeform SVG</span><small>Bézier/foreach • offline • V37.5.4</small></div><div class="v3745-stage">${r.svg}</div>${cap}${sourceDetails(normalize(item.figureLatex||''))}</div>`}
